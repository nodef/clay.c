...
#define CLAY_RENDERER_WIN32_GDI
#include <clay.h>
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam,
  ...
  case WM_MOUSEWHEEL: // scrolling data
  {
    ...
    // todo: i think GetMessageTime can roll over, so something like
    // if(lastmsgtime > now) ... may be needed
  ...
  case WM_PAINT: {
    Clay_RenderCommandArray renderCommands =
        ClayVideoDemo_CreateLayout(&demo_data);
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  Clay_Initialize(
      clayMemory, (Clay_Dimensions){.width = 800, .height = 600},
  ...
  fonts[FONT_ID_BODY_16] = Clay_Win32_SimpleCreateFont(
      "resources/Roboto-Regular.ttf", "Roboto", -11, FW_NORMAL);
